...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($username) || empty($email) || empty($password) || empty($confirm_password)) {
            ...
            $stmt = $pdo->prepare("SELECT id FROM users2 WHERE username = ? OR email = ?");
            ...
            if ($stmt->rowCount() > 0) {
                ...
                // Generate unique PH_id
                $ph_id = substr(md5(uniqid(rand(), true)), 0, 10);
                
                // Insert user into database
                $stmt = $pdo->prepare("INSERT INTO users2 (username, email, password, PH_id) VALUES (?, ?, ?, ?)");
                
                if ($stmt->execute([$username, $email, $hashed_password, $ph_id])) {
...
<head>
    ...
    <title>Register - TRON Wallet</title>
        ...
        <main class="flex justify-center">
            <div class="bg-background-card p-8 rounded-xl shadow-lg max-w-md w-full">
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                    ...
                    <div class="mb-4">
                        <label for="email" class="block text-text-secondary mb-1">Email</label>
                        <input type="email" id="email" name="email" class="w-full bg-background-dark border border-gray-700 rounded-lg px-4 py-2 focus:outline-none focus:border-accent-yellow" required>
                    </div>
                    ...
                    <div class="mb-6">
                        <label for="confirm_password" class="block text-text-secondary mb-1">Confirm Password</label>
                        <input type="password" id="confirm_password" name="confirm_password" class="w-full bg-background-dark border border-gray-700 rounded-lg px-4 py-2 focus:outline-none focus:border-accent-yellow" required>
                    </div>
                    
                    <button type="submit" class="w-full bg-accent-yellow text-black py-3 rounded-lg font-medium hover:bg-opacity-80 transition">Register</button>
                </form>
                
                <div class="mt-4 text-center text-text-secondary">
                    Already have an account? <a href="login.php" class="text-accent-yellow hover:underline">Login</a>
                </div>
            </div>
        </main>
    </div>
